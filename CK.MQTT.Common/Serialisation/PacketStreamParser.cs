...
namespace CK.MQTT.Common.Serialisation
{
    public static class PacketStreamParser
    {
        ...
        static IPacket? DeserialisePacketRequiringAlloc(
            ...
            ReadOnlyMemory<byte> buffer, IDisposable memoryHandle
        )
        {
            return packetType switch
            {
                PacketType.Connect => Connect.Deserialize( m, buffer, false, memoryHandle ),
                PacketType.Publish => Publish.Deserialise( m, header, buffer, memoryHandle ),
                _ => UnknownPacketType( m ),
            };
        }
        ...
        public static async ValueTask<(IPacket?, ReadOnlyMemory<byte>)> ReadPacket(
            IActivityMonitor m,
            Memory<byte> memoryBuffer,
            int alreadyReadCount,
            IGenericChannelStream stream,
            CancellationToken cancellationToken )
        {
            ...
            using( var grp = m.OpenDebug( "Reading incoming packet fixed header." ) )
            {
                ...
                if( !success ) return (null, memoryBuffer);
            }
            ...
            if( packetFullyInBuffer )//No alloc required path!
            {
                if( copyOnlyPacket )
                {
                    ...
                    return (packet, nextPacketBuffer);
                }
                if( packetSize == 0 )
                {
                    DeserialisePacketRequiringAlloc( m, packetType, header, ReadOnlyMemory<byte>.Empty, EmptyDisposable.Empty );
                }
            }
            ...
            using( m.OpenTrace( $"Deserialising incoming packet: type '{packetType}', size '{packetSize}' bytes (without fixed header)." ) )
            {
                if( copyOnlyPacket )
                {
                ...
                packet = DeserialisePacketRequiringAlloc( m, packetType, header, packetMemory, rentedMemory );
                return (packet, nextPacketBuffer);
            }
        }
    }
