...
namespace CK.MQTT.Common.Channels
{
    public class GenericChannel : IMqttChannel<IPacket>
    {
        ...
        async ValueTask PullToPush( Memory<byte> workBuffer )
        {
            ...
            {
                (IPacket? packet, ReadOnlyMemory<byte> buffer) =
        ...
        readonly SequentialEventHandlerSender<IMqttChannel<IPacket>, IPacket> _received = new SequentialEventHandlerSender<IMqttChannel<IPacket>, IPacket>();

        public event SequentialEventHandler<IMqttChannel<IPacket>, IPacket> Received
        {
            add => _received.Add( value );
            remove => _received.Remove( value );
        }
            ...
            where TReceive : class, IPacket
        {
            ...
            return (TReceive?)await _received.WaitAsync( TypePredicate, timeoutMillisecond );
        }
    }
}
