...
namespace CK.MQTT
{
    ...
    public delegate ValueTask<OperationStatus> ReflexMiddleware( IMqtt3Sink sink, InputPump sender, byte header, uint packetLength, PipeReader pipeReader, Func<ValueTask<OperationStatus>> next, CancellationToken cancellationToken );
    ...
    public interface IReflexMiddleware
    {
        ...
        ValueTask<OperationStatus> ProcessIncomingPacketAsync( IMqtt3Sink sink, InputPump sender, byte header, uint packetLength, PipeReader pipeReader, Func<ValueTask<OperationStatus>> next, CancellationToken cancellationToken );
    }
    ...
    public class ReflexMiddlewareBuilder
    {
        ...
        readonly struct Closure
        {
            ...
            public ValueTask<OperationStatus> RunAsync( IMqtt3Sink sink, InputPump s, byte h, uint l, PipeReader p, CancellationToken c )
            {
                Reflex previous = _previous;
                return _current( sink, s, h, l, p,
                () =>
                {
                    return previous( sink, s, h, l, p, c );
                }, c );//TODO: there is a closure here :|
            }
        }
    }
}
