...
namespace CK.MQTT
{
    public class PublishLifecycleReflex : IReflexMiddleware
    {
        ...
        public async ValueTask<OperationStatus> ProcessIncomingPacketAsync(
            IInputLogger? m, InputPump input, byte header, uint packetLength, PipeReader pipe, Func<ValueTask<OperationStatus>> next, CancellationToken cancellationToken )
        {
            ...
            using( m?.ProcessPacket( packetType ) )
            {
                ...
                {
                    case PacketType.PublishAck:
                        ushort? packetId2 = await pipe.ReadPacketIdPacketAsync( m, packetLength, cancellationToken );
                        if( !packetId2.HasValue ) return OperationStatus.NeedMoreData;
                        await _store.OnQos1AckAsync( m, packetId2.Value, null );
                        return OperationStatus.Done;
                    case PacketType.PublishComplete:
                        ushort? packetId3 = await pipe.ReadPacketIdPacketAsync( m, packetLength, cancellationToken );
                    ...
                    case PacketType.PublishRelease:
                        ...
                        ushort? packetId = await pipe.ReadPacketIdPacketAsync( m, packetLength, cancellationToken );
                        ...
                        return OperationStatus.Done;
                    case PacketType.PublishReceived:
                        ushort? packetId4 = await pipe.ReadPacketIdPacketAsync( m, packetLength, cancellationToken );
