...
namespace CK.MQTT
{
    class MemoryStoreFactory : IStoreFactory
    {
        ...
        public ValueTask<(PacketStore, IPacketIdStore)> CreateAsync( IActivityMonitor m, ProtocolConfiguration pConfig, MqttConfiguration config, string storeId, bool resetStore )
        {
            if( resetStore )
            {
