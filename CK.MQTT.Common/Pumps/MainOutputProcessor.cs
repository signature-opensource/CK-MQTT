...
namespace CK.MQTT.Common.Pumps
{
    public class MainOutputProcessor
    {
        ...
        public async ValueTask OutputProcessor( IOutputLogger? m, PacketSender packetSender, Channel<IOutgoingPacket> reflexes, Channel<IOutgoingPacket> messages, CancellationToken cancellationToken )
        {
            ...
            TimeSpan timeToNewPacket = await ResendUnackPacket( m, packetSender, messages, cancellationToken );//We send all packets that waited for too long.
            if( messageSent ) return; //We sent a packet, there is maybe more in stock, no need to go in the *heavy* wait logic !
            //No packet was sent, we need to wait a new one.
            //This part is heavy, but we don't care since the output pump have nothing else to do.

            //Here, if we are already waiting for PingResp
            Task keepAliveTask = Task.Delay( _pingRespReflex.WaitingPingResp ? _config.WaitTimeout : _config.KeepAlive, cancellationToken );//Cancellation token to cancel WhenAny.
            ...
            if( !_pingRespReflex.WaitingPingResp) //We 
            {
            ...
            if( !_pingRespReflex.WaitingPingResp && ) return; 
