...
namespace CK.MQTT
{
    public static class SmallOutgoingApplicationMessageExtensions
    {
        public static async ValueTask<Task> PublishAsync( this IMqtt3Client client, IActivityMonitor? m, string topic, QualityOfService qos, bool retain, ReadOnlyMemory<byte> payload )
        {
            ...
            return await client.SendPacketAsync<object>( m, new SmallOutgoingApplicationMessage( topic, qos, retain, payload ) );
        }
        ...
        {
            MqttBinaryWriter.ThrowIfInvalidMQTTString( topic );
            return await client.SendPacketAsync<object>( m, new SmallOutgoingApplicationMessage( topic, qos, retain, payload, responseTopic, correlationDataSize, correlationDataWriter ) );
        }
    }
}
