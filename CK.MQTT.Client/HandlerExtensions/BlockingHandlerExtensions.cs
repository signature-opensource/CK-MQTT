...
namespace CK.MQTT.Client.HandlerExtensions
{
    public static class BlockingHandlerExtensions
    {
        ...
        {
            client.SetMessageHandler( async ( string topic, PipeReader pipeReader, int payloadLength, QualityOfService qos, bool retain, CancellationToken cancellationToken ) =>
            {
        ...
        public static void CreateMQTT3Client( this MqttClientFactory factory, MqttConfiguration config, MemoryMessageHandlerDelegate messageHandler )
        {
            factory.CreateMQTT3Client( config, async ( string topic, PipeReader pipeReader, int payloadLength, QualityOfService qos, bool retain, CancellationToken cancellationToken ) =>
            {
                 using( IMemoryOwner<byte> memoryOwner = MemoryPool<byte>.Shared.Rent( payloadLength ) )
                 {
                     Memory<byte> buffer = memoryOwner.Memory[..payloadLength];
