...
namespace CK.MQTT.Client.HandlerExtensions
{
    public static class BlockingHandlerExtensions
    {
        ...
        {
            async ValueTask messageHandler1( string topic, PipeReader pipeReader, int payloadLength, QualityOfService qos, bool retain, CancellationToken cancellationToken )
            {
            ...
            client.SetMessageHandler( (string topic, PipeReader pipeReader, int payloadLength, QualityOfService qos, bool retain, CancellationToken cancellationToken) =>
            {
                using( IMemoryOwner<byte> memoryOwner = MemoryPool<byte>.Shared.Rent( payloadLength ) )
                {
                    Memory<byte> buffer = memoryOwner.Memory[..payloadLength];
