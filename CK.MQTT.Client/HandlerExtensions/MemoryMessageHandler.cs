...
namespace CK.MQTT
{
    public static class MemoryMessageHandler
    {
        ...
        {
            readonly Func<string, ReadOnlyMemory<byte>, QualityOfService, bool, CancellationToken, ValueTask> _messageHandler;
            public HandlerCancellabeClosure( Func<string, ReadOnlyMemory<byte>, QualityOfService, bool, CancellationToken, ValueTask> messageHandler )
            {
        ...
        public static void SetMessageHandler( this IMqtt3Client client, Func<string, ReadOnlyMemory<byte>, QualityOfService, bool, CancellationToken, ValueTask> messageHandler )
        ...
        public static IMqtt3Client CreateMQTT3Client( this MqttClientFactory @this,
            MqttConfiguration config, Func<string, ReadOnlyMemory<byte>, QualityOfService, bool, CancellationToken, ValueTask> messageHandler )
        ...
        {
            readonly Func<string, ReadOnlyMemory<byte>, QualityOfService, bool, ValueTask> _messageHandler;
            public HandlerClosure( Func<string, ReadOnlyMemory<byte>, QualityOfService, bool, ValueTask> messageHandler )
                => _messageHandler = messageHandler;
            ...
            public async ValueTask HandleMessage( string topic, PipeReader pipe, int payloadLength, QualityOfService qos, bool retain, CancellationToken cancelToken )
            {
                using( IMemoryOwner<byte> memoryOwner = MemoryPool<byte>.Shared.Rent( payloadLength ) )
                {
                    ...
                    PipeReaderExtensions.FillStatus res = await pipe.CopyToBuffer( buffer, cancelToken );
                    if( res != PipeReaderExtensions.FillStatus.Done ) throw new EndOfStreamException();
                    await _messageHandler( topic, buffer, qos, retain );
                }
            }
        }

        public static void SetMessageHandler( this IMqtt3Client client, Func<string, ReadOnlyMemory<byte>, QualityOfService, bool, ValueTask> messageHandler )
            => client.SetMessageHandler( new HandlerClosure( messageHandler ).HandleMessage );

        public static IMqtt3Client CreateMQTT3Client( this MqttClientFactory factory, MqttConfiguration config, Func<string, ReadOnlyMemory<byte>, QualityOfService, bool, ValueTask> messageHandler )
            => factory.CreateMQTT3Client( config, new HandlerClosure( messageHandler ).HandleMessage );

        public static IMqtt5Client CreateMQTT5Client( this MqttClientFactory factory, MqttConfiguration config, Func<string, ReadOnlyMemory<byte>, QualityOfService, bool, ValueTask> messageHandler )
            => factory.CreateMQTT5Client( config, new HandlerClosure( messageHandler ).HandleMessage );

        public static IMqttClient CreateMQTTClient( this MqttClientFactory factory, MqttConfiguration config, Func<string, ReadOnlyMemory<byte>, QualityOfService, bool, ValueTask> messageHandler )
