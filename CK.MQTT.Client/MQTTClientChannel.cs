...
{
    public abstract class MQTTClientChannel : Mqtt3ClientBase
    {
        ...
        public ChannelReader<object?>? Messages { get; private set; }

        [MemberNotNull( "Messages", "_channel" )]
        public override async Task<ConnectResult> ConnectAsync( OutgoingLastWill? lastwill = null, CancellationToken cancellationToken = default )
        {
            ...
            _channel = Channel.CreateUnbounded<object?>( new UnboundedChannelOptions() { SingleReader = true, SingleWriter = false } );
            Messages = _channel.Reader;
        ...
        public override async Task<bool> DisconnectAsync( bool deleteSession )
        {
            if( !IsConnected ) throw new InvalidOperationException( "Already disconnected." );
        ...
        protected override async ValueTask ReceiveAsync( string topic, PipeReader pipe, uint payloadLength, QualityOfService qos, bool retain, CancellationToken cancellationToken )
        {
            ...
            await _channel!.Writer.WriteAsync( new ApplicationMessage( topic, buffer, qos, retain ), cancellationToken ); //Todo: DisposableApplicationMessage
        }
