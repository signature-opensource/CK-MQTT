...
namespace CK.MQTT
{
    ...
    public interface IMqtt3Client : IDisposable
    {
        ...
        void SetMessageHandler( Func<IActivityMonitor, string, PipeReader, uint, QualityOfService, bool, CancellationToken, ValueTask> messageHandler );
        ...
        /// </remarks>
        Task<ConnectResult> ConnectAsync( IActivityMonitor? m, MqttClientCredentials? credentials = null, OutgoingLastWill? lastWill = null, CancellationToken cancellationToken = default );

        ValueTask<Task<T?>> SendPacketAsync<T>( IActivityMonitor? m, IOutgoingPacket outgoingPacket ) where T : class;
        ...
        /// </remarks>
        Task<bool> DisconnectAsync( IActivityMonitor? m, bool deleteSession, bool cancelAckTasks, CancellationToken cancellationToken );
    }
